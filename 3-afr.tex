...
Furthermore, parametric distributions are known to be robust to omitted covariates~\citep{lambert2004parametric}.


\cm{Introductory statistics textbooks~\ref{stats} begin the discussion of failure modelling quite simply by using the exponential distribution, which models the probability of an event occurring at a certain time when there is a constant failure rate~\ref{stats}. Likewise, the gamma distribution can be used to calculate the time between failures when the failure rate is constant~\ref{stats}. Kleinbaum notes that survival time, failure rate, and probability of failure can all be calculated if you know one of these and the data can be fit by one of the described models. This author also notes the efficacy of the Cox semi-parametric approach where:}
$$
S_\theta(t) = S_0(t) \exp(\theta_1 x1 + \theta_2 x_2 + \cdots + \theta_p x_p)
$$
\cm{Unlike the aforementioned models (\textit{e.g.}~ exponential, gamma, and cox), we also examine the efficacy of another class of methods called accelerated failure time models which assume that we can express the survival time as,
$$
...
\]
\cm{Unlike the exponential and gamma distributions that assume a constant failure rate, both the Cox and AFR models can be fit to time-variant data. However, unlike the Cox model, the AFR models listed below are fully parametric and the }Furthermore, parametric models are known to be robust to omitted covariates~\citep{lambert2004parametric}.
% While non-parametric AFR models like Cox's \textbf{proportional hazard model} do exist, they don't allow us to estimate the underlying distribution~\cite{kleinbaum1996survival} --- merely the proportional effects of the covariates~\citep{aft_models}. 
