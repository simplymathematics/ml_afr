...
% . Estimating the expected survival time of a model requires a choice of probability distribution ($\mathcal{X}$) \footnote{While non-parametric AFR models like Cox's \textbf{proportional hazard model} do exist, they don't allow us to estimate the underlying distribution~\cite{kleinbaum1996survival} --- merely the proportional effects of the covariates~\citep{aft_models}. Furthermore, parametric distributions are known to be robust to omitted covariates~\citep{lambert2004parametric}.}, and we outline the most common ones in the subsections below. The expected value of a random variable, $\mathcal{X}$:
%  \[
%  \mathbb{E}[\mathcal{X}] = \mathbb{P}(\mathcal{X} > t) = \int_0^{\infty} t \cdot S(t) dt
%  \]
%  where $\mathbb{E}$ is the expectation, and $\mathbb{P}(\mathcal{X} > t)$ is the probability of drawing a sample from $\mathcal{X}$ with survival time greater than $t$.
...
\begin{equation}
    ...
    \text{~~~and~~~}
    \bar{C}_{adv.} = \frac{t_{train}}{\mathbb{E}_{S_\theta}[T \,|\, 0 < \varepsilon \leq \varepsilon_{max}]}.
    \label{eq:cost}
\end{equation}
