...
\begin{figure}[!h]
    ...
    \end{subfigure}
    \caption{This figure depicts the TRASH metric that reflects the ratio of training-to-attack times, where a value$\gg 1 $  indicates an essential advantage for the attacker. The violin plots reflect the 95\% confidence intervals for each tuned hyperparameter combination. Outliers are indicated with a circle. \cm{Note for Tommy/Erik: During the last iteration, the metric used the total training time rather than the training time per sample in the numerator. I think either formulation would be totally valid since we train on 10s of thousands of samples and not just one sample, though the interpretation is slightly different. Anyway, the y-axis subsequently shifted down by 3 orders of magnitude (by dividing the numerator by 48000 training samples and the denominator by 100 attack samples).}}
